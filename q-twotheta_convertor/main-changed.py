...
class XRDReaderPlotQ(QMainWindow):
    ...
    def init_ui(self):
        ...
        self.save_btn.clicked.connect(self.save_data_as_xy)
    ...
    def read_xrd_data(self, file_path):
        self.data = pd.read_csv(file_path, sep='\s+', header=None, names=['Q', 'Intensity'], error_bad_lines=False)
        wavelength = float(self.wavelength_input.text())  # 从输入框获取波长值
        self.data['2Theta'] = self.data['Q'].apply(lambda q: self.convert_q_to_2theta(q, wavelength))
    ...
    def plot_data(self):
        if self.data is not None:
            ...
            ax.plot(self.data['2Theta'], self.data['Intensity'], marker='o', linestyle='-')
    ...
    def save_data_as_xy(self):
        if self.data is not None:
            ...
            if output_file_path:
                ...
                self.data[['2Theta', 'Intensity']].to_csv(output_file_path, sep='\t', header=None, index=None)
